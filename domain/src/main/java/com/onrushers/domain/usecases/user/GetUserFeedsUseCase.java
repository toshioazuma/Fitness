...
public class GetUserFeedsUseCase extends UseCase implements GetUserFeedsInteractor {
	...
	private FeedType mFeedType  = FeedType.Any;

	@Inject
	public GetUserFeedsUseCase(AuthSessionRepository authSessionRepository,
		...
		return mAuthSessionRepository.getAuthSession()
			.flatMap(new Func1<IAuthSession, Observable<IFeedPagination>>() {
				...
				public Observable<IFeedPagination> call(IAuthSession authSession) {
					...
					final int loggedUserId = authSession.getUserId();
					final int userId;
					...
					return mUserRepository
						.getUserFeeds(userId, mPage, mPageCount, mFeedType, authSession.getToken())
